...
results_df <- results_df %>% 
  group_by(variable) %>%
  mutate(
    slope = round(scale(slope, center=FALSE), 2)  # divide by standard deviation, do not center to preserve signs
  ) %>%
  ungroup()
```
...
Before building a statistical or machine learning model, it is important to establish a simple "strawman" model as a baseline for comparison to any more advanced methods. We do this by first developing a simple forecasting model using only *HPI* without the use of the exogenous variables. From plots of HPI, it appears as though there is a strong trend over time. We employ and Augmented Dickey-Fuller test for a unit root. 

Specifically, we test whether $\alpha$ and $\gamma$ in $\Delta y_t = \alpha + \gamma y_{t-1} + e_t$ are 0. The null hypothesis for the ADF test is that $\gamma=0$. The results of the test are shown below. Specifically, we fail to reject the null that $\gamma = 0$ since the test statistic -0.3024 is above the critical values. However, we reject the null hypothesis that both $\alpha = \gamma= 0$ since the test statistic 1.2716 is below the critical values. Essentially the null hypothesis is that the series follows a random walk with drift (not that we are not saying that HPI *does* follow a random-walk with drift, only that we can say it is not stationary and the drift term is positive and significant).

```{r}

# StackOverflow post for interpreting the output of the ur.df function: https://stats.stackexchange.com/questions/24072/interpreting-rs-ur-df-dickey-fuller-unit-root-test-results

adf_test <- urca::ur.df(data_ts[, "HPI"], lag=0, type="drift")
summary(adf_test)

```

Based on the conclusion of the ADF test, we decide to make a random walk with drift our "strawman" model against which we will compare other models. The results are shown below and we see that random-walk forecast is not very good. Hopefully we can beat it with more sophisticated models. 
...
This suggests that we include two lagged autoregressive terms in the model and in the error structure: 
$$
HPI_t = \beta_0 + \phi_1 HPI_{t-1} + \phi_2 HPI_{t-2} + \beta_1 \text{purch_orig_gdp}_t + \beta_2 \text{refi_orig_gdp}_t + \beta_3 t + e_t
$$

where $e_t = \delta_0 + \delta_1 e_{t-1} + \delta_2 e_{t-2}$. (Note that the autoregressive structure in the errors hints at a dynamic process that is unaccounted for by contemporaneous regressors and could imply we should try lagged values of the exogenous variables or include more/different variables in the model).
...
arima_model <- Arima(y=train_ts[, "HPI"], xreg=X_train, order=c(2,0,2), include.drift = TRUE)
...
autoplot(fcast_arima) + xlab("Year") + ylab("HPI") + ggtitle("ARIMA(2,0,2) with Drift and Exogenous Regressors", subtitle=eval(STATE)) + 
  autolayer(test_ts[,"HPI"], series="Actual")
...
Use the model developed above to forecast the level and percent change in HPI for the three years 2018 through 2020. 
